...
import { getProjectLanguageInfo, getProjectRoot, isInProject } from "./utils/project.js";
	...
	async run() {
		...
		if (hasComponentsConfig()) {
			const shouldOverwrite = await p.confirm({
				message: "Method UI is already initialized. Do you want to overwrite the configuration?",
				initialValue: false,
			});
		...
		if (missingPackages.length > 0) {
			...
			if (shouldInstall) {
				...
				p.log.warn("Please install the required packages manually before proceeding.");
				process.exit(1);
			}
		} else {
		...
		p.log.message(`  Language: ${config.typescript ? "TypeScript" : "JavaScript"}`);
		...
		if (confirm) {
			try {
				...
				p.log.message(`  • uno.config.${config.typescript ? "ts" : "js"} - UnoCSS configuration`);
...
const addCommand = defineCommand({
	...
	async run({ args }) {
		...
		if (componentNames.length === 0) {
			...
			const invalidComponents = componentNames.filter((name) => !componentExists(name));
			...
			try {
				...
				if (result.success) {
					if (result.installedComponents.length > 0) {
						p.log.success(`Successfully added ${result.installedComponents.length} component(s):`);
					...
					if (result.installedPackages.length > 0) {
						p.log.success(`Successfully installed ${result.installedPackages.length} package(s):`);
					...
					if (result.installedComponents.length > 0 || result.installedPackages.length > 0) {
...
const removeCommand = defineCommand({
	...
	async run({ args }) {
		...
		if (!existsSync(componentPath)) {
			p.log.error(`Component "${args.component}" not found at ${componentsPath}/${componentFile}`);
