...
export default defineConfig({
  // Custom extractor to find icon() function calls
  extractors: [
    {
      name: "icon-function-extractor",
      extract({ code }) {
        // Match icon("name") or icon('name') calls
        const iconRegex = /icon\(['"]([^'"]+)['"]\)/g;
        const matches = [...code.matchAll(iconRegex)];

        // Generate icon classes from matches
        const classes = matches.map((match) => `i-lucide-${match[1]}`);
