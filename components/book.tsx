...
const bookVariants = cva(
  "group inline-block w-fit cursor-pointer transition-all duration-300",
  {
    variants: {
      animation: {
        hover:
          "[&>div]:hover:[transform:rotateY(-25deg)_scale(1.05)_translateX(-12px)]",
        default:
          "[&>div]:hover:[transform:rotateY(-20deg)_scale(1.066)_translateX(-8px)]",
        none: "",
      },
      size: {
        xs: "",
...
export const Book: Component<BookProps> = (props) => {
  ...
  const dimensions = () => sizePresets[local.size!];
