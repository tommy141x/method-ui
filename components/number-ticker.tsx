...
export const NumberTicker: Component<NumberTickerProps> = (props) => {
	...
	const easeOut = (t: number) => 1 - (1 - t) ** ease();
			...
			(entries) => {
				entries.forEach((entry) => {
					if (entry.isIntersecting && !hasStarted()) {
						...
						setHasStarted(false);
						setDisplayValue(direction() === "down" ? local.value : startValue());
						if (rafId) {
	...
	return (
		<Show
			...
			fallback={
				<span class={cn("inline-block tabular-nums tracking-wider", local.class)} {...others}>
	...
	examples: [
		{
			...
			code: () => (
				<div class="flex flex-col items-center gap-4 p-8">
					...
					<p class="text-sm text-muted-foreground">Number animates when scrolled into view</p>
				</div>
			),
		},
		{
			...
			code: () => (
				<div class="flex flex-col items-center gap-4 p-8">
					<div class="text-center">
						<NumberTicker value={99.99} decimalPlaces={2} class="text-5xl font-bold" />
			...
			code: () => (
				<div class="grid grid-cols-1 md:grid-cols-3 gap-6 p-8">
					<div class="text-center">
						...
						<NumberTicker value={5000} duration={1000} class="text-4xl font-bold" />
					</div>
					<div class="text-center">
						<p class="text-sm text-muted-foreground mb-2">Slow (3s)</p>
						<NumberTicker value={5000} duration={3000} class="text-4xl font-bold" />
					</div>
					<div class="text-center">
						<p class="text-sm text-muted-foreground mb-2">Slow + Strong Ease (3s)</p>
						<NumberTicker value={5000} duration={3000} ease={8} class="text-4xl font-bold" />
					</div>
				</div>
			),
		},
