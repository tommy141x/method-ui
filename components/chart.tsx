...
const deepMerge = (target: unknown, source: unknown): Record<string, unknown> => {
...
export const Chart: Component<ChartProps> = (props) => {
	...
	const [chartInstance, setChartInstance] = createSignal<echarts.ECharts | undefined>(undefined);
	...
	const mergedOption = createMemo((): ECOption => {
		...
		if (base.timeline) merged.timeline = deepMerge(theme.timeline, base.timeline);
		if (base.visualMap) merged.visualMap = deepMerge(theme.visualMap, base.visualMap);
		if (base.dataZoom) merged.dataZoom = deepMerge(theme.dataZoom, base.dataZoom);
		if (base.markPoint) merged.markPoint = deepMerge(theme.markPoint, base.markPoint);
					...
					if (s.type === "gauge") {
						return {
							...
							pointer: {
								...
								itemStyle: {
									...s.pointer?.itemStyle,
									color: s.pointer?.itemStyle?.color || getCSSVariable("--primary"),
								},
							},
							axisTick: {
								...s.axisTick,
								lineStyle: {
									...
									color: s.axisTick?.lineStyle?.color || getCSSVariable("--primary"),
								},
							},
							splitLine: {
								...s.splitLine,
								lineStyle: {
									...
									color: s.splitLine?.lineStyle?.color || getCSSVariable("--primary"),
								},
							},
							axisLabel: {
								...s.axisLabel,
	...
	onMount(() => {
		...
		const initChart = () => {
			...
			const themeToUse = props.theme || (props.useTheme !== false ? dynamicTheme() : undefined);
	...
	return (
		<div
			ref={containerRef}
			id={props.id}
			class={cn(chartVariants({ variant: props.variant, size: props.size }), props.class)}
			style={props.style}
		/>
	);
};
	...
	examples: [
		{
			...
			code: () => {
				const initialData = Array.from({ length: 20 }, () => Math.floor(Math.random() * 100) + 20);
		...
		},
		{
			...
			description: "Hierarchical tree visualization of an organization structure",
			code: () => {
				return (
					<Chart
						size="lg"
											...
											children: [
												{
													...
													children: [
														...
														{
															name: "Product",
															children: [{ name: "Design Team" }, { name: "Research Team" }],
														},
													],
												},
												{
													...
													children: [
														{
															name: "Finance",
															children: [{ name: "Accounting" }, { name: "Treasury" }],
														},
														{
															name: "Legal",
												...
												},
												{
													...
													children: [
														...
														{
															name: "Sales",
															children: [{ name: "Enterprise Sales" }, { name: "SMB Sales" }],
														},
													],
												},
											],
