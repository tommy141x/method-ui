...
const awardVariants = cva("", {
	variants: {
		variant: {
			stamp: "relative mx-auto flex h-48 w-48 items-center justify-center",
			award: "relative z-0 flex flex-col items-center justify-center p-6 overflow-hidden",
			certificate:
				"relative z-0 flex flex-col items-center justify-center rounded-xl border-2 border-dotted p-2 overflow-hidden",
			badge: "",
		},
	},
	defaultVariants: {
...
export interface AwardProps {
	variant?: VariantProps<typeof awardVariants>["variant"];
	title: string;
	subtitle?: string;
	description?: string;
	date?: string;
...
const StampVariant: Component<AwardProps> = (props) => {
	return (
		<div class={cn(awardVariants({ variant: "stamp" }), props.class)}>
			<svg
			...
			>
				...
				<path d={createSerratedPath()} stroke-width="0.2" class="fill-card stroke-foreground" />

				<circle cx="96" cy="96" r="78" class="fill-card stroke-foreground" stroke-width="0.2" />
				...
				<text class="text-[10px] tracking-wider">
					<textPath
						href="#bottom-curve"
						startOffset="50%"
						text-anchor="middle"
			...
			<div class="relative z-10 text-center">
				...
				{props.date && <div class="text-[10px] italic">{props.date}</div>}
			</div>
		</div>
	);
};

const LaurelWreath: Component = () => (
	<svg
		class="fill-primary absolute top-1/2 h-full w-full -translate-y-1/2"
		width="892.77"
		height="688.08"
...
const AwardVariant: Component<AwardProps> = (props) => {
	return (
		<div class={cn(awardVariants({ variant: "award" }), props.class)}>
			<LaurelWreath />
			<div class="z-10 px-8 text-center">
				<div
					class={cn(
						"mt-6 mb-2 inline-block rounded-md px-4 py-1 tracking-wider text-white",
						`bg-linear-to-r ${levelColors[props.level || "gold"]}`
					)}
				>
					{(props.level || "gold").toUpperCase()}
				</div>

				<h1 class="text-4xl font-black tracking-tight">{props.title}</h1>

				<div class="bg-primary mx-auto my-3 h-px w-40"></div>

				<h2 class="mb-4 w-60 text-xl font-light">{props.subtitle}</h2>

				{props.recipient && <p class="text-primary/60 italic">{props.recipient}</p>}

				<div class="text-xl font-bold">{props.date}</div>
			</div>
		</div>
	);
};

const CertificateBadgeSvg: Component = () => (
	<svg
		class="fill-primary -mt-12 h-18 w-full overflow-hidden"
		width="24"
		height="24"
		viewBox="0 0 24 24"
		aria-hidden="true"
...
const CertificateVariant: Component<AwardProps> = (props) => {
	return (
		<div class={cn(awardVariants({ variant: "certificate" }), props.class)}>
			<div class="bg-card z-10 rounded-sm border p-6 px-8 text-center">
				<CertificateBadgeSvg />
				<h1 class="mt-4 grid text-3xl leading-7 font-bold tracking-tighter uppercase">
					Certificate
					<span class="text-sm font-light tracking-tight">of {props.title}</span>
				</h1>

				<p class="text-muted-foreground mt-4 mb-1 text-xs">This is to certify that</p>
				<h1 class="text-primary mb-2 border-b text-xl font-semibold tracking-tight">
					{props.recipient}
				</h1>
...
const BadgeSvg: Component = () => (
	<svg
		class="fill-primary h-full w-18 overflow-hidden"
		width="500.15"
		height="620.78"
...
const BadgeVariant: Component<AwardProps> = (props) => {
	return (
		<div class={cn(awardVariants({ variant: "badge" }), props.class)}>
			<div class="rounded-md border-4 p-4 flex justify-start gap-3">
				<BadgeSvg />
				<div class="border-l px-3">
					<h1 class="text-primary text-4xl font-bold tracking-tight">{props.title}</h1>
					<h2 class="text-muted-foreground text-md font-light">{props.subtitle}</h2>

					<div class="mt-1 flex items-center gap-4 text-xs">
						{props.recipient && <p class="italic">by {props.recipient}</p>}
						{props.recipient && props.date && "•"}
...
export const Award: Component<AwardProps> = (props) => {
	...
	const variant = local.variant || "award";

	if (variant === "stamp") {
		return <StampVariant {...local} {...others} />;
	}

	if (variant === "certificate") {
		return <CertificateVariant {...local} {...others} />;
	}

	if (variant === "badge") {
		return <BadgeVariant {...local} {...others} />;
	}

	return <AwardVariant {...local} {...others} />;
};

export const meta: ComponentMeta<AwardProps> = {
	name: "Award",
	description:
		"A versatile award component with multiple variants including stamps, awards with laurel wreaths, certificates, and badges. Perfect for displaying achievements, recognitions, and accomplishments.",
	apiReference: "",
	variants: awardVariants,
	examples: [
		{
			title: "Award Variants",
			description: "Different award styles",
			code: () => (
				<div class="grid grid-cols-2 gap-8 w-full">
					...
					<Award
						variant="certificate"
						title="Achievement"
						subtitle="For demonstrating exceptional skills"
		...
		{
			title: "Stamp",
			description: "Circular stamp style award",
			code: () => (
		...
		{
			title: "Badge",
			description: "Horizontal badge style",
			code: () => (
		...
		{
			title: "Award Levels",
			description: "Different achievement levels",
			code: () => (
		...
		{
			title: "Certificate",
			description: "Formal certificate style",
			code: () => (
				<Award
					variant="certificate"
					title="Completion"
					subtitle="has successfully completed the advanced training program with distinction"
