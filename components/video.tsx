...
const plyrThemeStyles: Record<string, string> = {
  ...
  "--plyr-video-controls-background":
    "linear-gradient(transparent, hsl(var(--background) / 0.95))",
...
export const Video: Component<VideoProps> = (props) => {
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (player && player.on) {
        if (local.onReady) {
          player.on("ready", (event: any) => local.onReady?.(player));
        }
        if (local.onPlay) {
      ...
      if (player && player.on) {
        player.on("error", (event: any) => {
          ...
          setError(
            "Failed to load video. The video source may be unavailable.",
          );
