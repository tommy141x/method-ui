...
export const TextGenerateEffect: Component<TextGenerateEffectProps> = (props) => {
	...
	return (
		...
		>
			<div ref={setElementRef} class={cn("leading-snug tracking-wide", local.class)} {...others}>
				<For each={wordsArray()}>
					{(word, i) => (
						<span
							...
							style={{
								...
								transform: visibleWords()[i()] ? "translateY(0px)" : "translateY(10px)",
								filter: filter() && !visibleWords()[i()] ? "blur(10px)" : "blur(0px)",
		...
		},
		{
			...
			code: () => (
				<div class="flex flex-col gap-8 p-8">
					<div>
						<p class="text-sm text-muted-foreground mb-2">Fast (100ms stagger)</p>
						<TextGenerateEffect
					...
					<div>
						<p class="text-sm text-muted-foreground mb-2">Slow (400ms stagger)</p>
						<TextGenerateEffect
