					...
					>
						...
						>
							...
							<circle
								...
								class={cn("text-primary transition-opacity", props.disabled && "opacity-50")}
								style={{
						...
						{markers.length > 0 && (
							...
							>
								{markers.map((value) => {
									...
									const x = centerX + Math.cos(angle) * markerRadius - markerSize / 2;
									const y = centerY + Math.sin(angle) * markerRadius - markerSize / 2;
									return (
							...
							{(context) => (
								<div class="absolute inset-0 flex items-center justify-center pointer-events-none">
									{typeof props.centerValue === "boolean" ? (
										<div class="text-4xl font-bold text-primary">{context().value}°</div>
...
export const meta: ComponentMeta<AngleSliderProps> = {
	...
	examples: [
		{
			title: "Basic",
			description: "A simple angle slider with the value displayed in the center",
			code: () => {
				return (
					<AngleSlider
