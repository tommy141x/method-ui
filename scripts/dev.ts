...
async function generateMetadata(changedFile?: string) {
	if (isGenerating) {
		log(colors.yellow, "WATCHER", "Generation already in progress, skipping...");
		return;
	}
	...
	log(colors.magenta, "WATCHER", `Change detected${fileInfo}, regenerating metadata...`);
...
function startWatcher() {
	...
	watcher
		...
		.on("ready", () => {
			log(colors.green, "WATCHER", "✨ Initial scan complete. Ready for changes!");
		});
}
