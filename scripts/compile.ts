 ...
 */
function extractVariantsFromMeta(sourceCode: string, _sourceFile: ts.SourceFile): VariantDoc[] {
	...
	while (match !== null) {
		...
		for (const valueMatch of valueMatches) {
			...
			if (!valueName.match(/^(hover|focus|active|disabled|group|peer|before|after|first|last)/)) {
...
function extractExamplesFromMeta(sourceCode: string): ExampleDoc[] {
	...
	for (const element of examplesArray.elements) {
		...
		if (ts.isParenthesizedExpression(codeFunction.body)) {
			...
			const hasVariableDeclarations = codeFunction.body.statements.some((stmt) =>
...
function extractDescription(sourceCode: string): string | undefined {
	const descRegex = /export\s+const\s+meta[^=]*=\s*\{[\s\S]*?description:\s*["']([^"']+)["']/;
 ...
 */
function extractDependencies(sourceCode: string, fileName: string): DependencyInfo {
	...
	for (const line of lines) {
		...
		// Skip comments
		else if (trimmed.startsWith("//") || trimmed.startsWith("*") || inBlockComment) {
		}
...
export async function generateComponentMetadata(specificFile?: string) {
	...
	if (specificFile) {
		...
		allMetadata[fileName] = processComponent(specificFile, componentsDir, false);
	} else {
		...
		const results = await Promise.all(
			files.map(async (file) => {
				const fileName = file.replace(".tsx", "");
				const metadata = processComponent(file, componentsDir, true, sharedProgram);
